...
inotifywait -m -q -r --exclude $WATCH_EXCLUDE --format '%w/%f--%e' $WATCH_DIR | \
  while read -r FILE
  do
    ...
    if [ -z "$PID" ]; then
	  ...
	  ($COMMAND; while read -t0.001 -u3 LINE 2>/dev/null; do
	    echo running >&4
	    $COMMAND
	  done)&
      ...
      if [ $WAITING -eq 1 ] && read -t0.001 -u4 2>/dev/null; then
        WAITING=0
      fi
